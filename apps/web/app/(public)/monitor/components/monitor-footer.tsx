...
interface FidsFooterProps {
  message?: string;
}

export function FidsFooter({ 
  message = "Selamat datang di Area Vendor  •  Harap tunggu hingga jam operasional sesuai dengan jadwal Anda  •  Pastikan kelengkapan dokumen Surat Jalan dan Identitas Anda siap" 
}: FidsFooterProps) {
  return (
    <footer className="relative bg-primary h-14 shrink-0 overflow-hidden">
      ...
      <div className="absolute left-48 right-48 top-0 bottom-0 flex items-center overflow-hidden">
        <div className="flex animate-marquee whitespace-nowrap">
          <span className="flex items-center gap-4 text-lg font-medium text-primary-foreground tracking-wide px-8">
            <Truck className="w-5 h-5 text-primary-foreground/70 shrink-0" />
            {message}
          </span>
          <span className="flex items-center gap-4 text-lg font-medium text-primary-foreground tracking-wide px-8">
            <Truck className="w-5 h-5 text-primary-foreground/70 shrink-0" />
      ...
      <div className="absolute right-0 top-0 bottom-0 flex items-center gap-2 px-6 bg-black/20 border-l border-primary-foreground/10 z-10">
        <span className="font-medium text-primary-foreground text-sm whitespace-nowrap">PT TMMIN</span>
      </div>
